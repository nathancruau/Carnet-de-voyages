...
function _mmShowInfo(trip, entry) {
  ...
  // GPX stats + chart (shared with the sortie detail screen) + draw polyline on map
  const gpxHtml = entry.gpxStats ? gpxStatsBlockHtml('mm', entry.gpxStats, entry.gpxPoints) : '';
  if (entry.gpxStats) {
  ...
  // Init GPX chart after DOM is ready (no-op if there's no canvas to wire up)
  if (entry.gpxPoints?.length >= 2) initGpxChart('mm', entry.gpxPoints);
}
